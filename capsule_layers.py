...
class ConvCapsuleLayer(layers.Layer):
    ...
    def build(self, input_shape):
        ...
        self.W = self.add_weight(shape=[self.kernel_size, self.kernel_size,
                                 ...
                                 name='W')
        self.b = self.add_weight(shape=[1, 1, self.num_capsule, self.num_atoms],initializer=initializers.constant(0.1),name='b')
        self.built = True
    ...
    def call(self, input_tensor, training=None):
        ...
        input_tensor_reshaped = K.reshape(input_transposed, [input_shape[0] * input_shape[1], self.input_height, self.input_width, self.input_num_atoms])
        input_tensor_reshaped.set_shape((None, self.input_height, self.input_width, self.input_num_atoms))
        dilation_rate = math.ceil((self.input_height - 1) / 4)
        Conv = K.conv2d(input_tensor_reshaped, self.W, (self.strides, self.strides), padding=self.padding,dilation_rate=dilation_rate,
                        data_format='channels_last')
        votes_shape = K.shape(Conv)
        _, conv_height, conv_width, _ = Conv.get_shape()
        votes = K.reshape(Conv, [input_shape[1], input_shape[0], votes_shape[1], votes_shape[2],self.num_capsule, self.num_atoms])
        ...
        bias = K.tile(self.b, [conv_height, conv_width, 1, 1])
        activations = routing(votes=votes,biases=bias,logit_shape=logit_shape,num_dims=6,input_dim=self.input_num_capsule,
...
def routing(votes, biases, logit_shape, num_dims, input_dim, output_dim,num_routing):
    if num_dims == 6:
